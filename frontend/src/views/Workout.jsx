...
import { effectiveRoutine, effectiveRoutines, lastEntryFor, bestWeightFor, buildSets, setsDoneActive, supersetUnits, unitOf, setLabel, modeOf, isBw, isPerSide, sideReps, repStep, EFFORT, effortOf, stepEffort, capEffort } from '../lib/history.js'
...
function ExerciseBlock({ entryIdx, compact, onToggle, onField, onAddSet, onRemoveSet, onStartTimed }) {
  ...
  return <>
    ...
    {last && <div className="small dim" style={{ marginBottom: 4 }}>{t('Last time')} ({fmtDate(last.d, false, S.calendar || 'gregorian')}): {last.sets.map(s => setLabel(entry.id, s, last.target)).join(', ')}</div>}
